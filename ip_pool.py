...
class IPFactory:
    ...
    def __init__(self):
        ...
        self.all_ip = set()
        self.create_db()
        self.ip_pool = self.get_useful_proxies()
        # 创建数据库
    ...
    def create_db(self):
        ...
        # 创建数据库/表语句
        # 创建数据库
        drop_table_str = 'drop table'+ ' if exists '+cfg.TABLE_NAME +  ';'
        # 创建表格
        ...
        # 连接数据库
        conn = sqlite3.connect(cfg.database_name)
        cursor = conn.cursor()
        try:
            cursor.execute(drop_table_str)
            cursor.execute(create_table_str)
            conn.commit()
        except OSError:
            logging.info("无法创建数据库！")
        finally:
            cursor.close()
            conn.close()
    ...
    def get_content(self, url, url_xpath, port_xpath):
        ...
        try:
            ...
            logging.info('get proxies error: ', e)
        ...
        for p in ip_set:
            ...
            try:
                ...
                if r.text is not None:
                    logging.info('succeed: ' + p + '\t' + " in " + format(end-start, '0.2f') + 's')
                    # 追加代理ip到返回的set中
                    results.add(p)
            except OSError:
                logging.info('timeout:', p)
        ...
        try:
            for item in valid_ips:
                ...
                if item_exist == 0:
                    ...
                    # 输出入库状态
                    if n:
                        logging.info(datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")+" "+item+" 插入成功。\n")
                    else:
                        logging.info(datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")+" "+item+" 插入失败。\n")

                else:
                    logging.info(datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")+" "+ item +" 已存在。\n")
        except Exception as e:
            logging.warning("入库失败：" + str(e))
        finally:
            cursor.close()
            conn.close()
        logging.info("\n>>>>>>>>>>>>>>>>>>>> 代理数据入库处理 End  <<<<<<<<<<<<<<<<<<<<<<\n")
    ...
    def get_useful_proxies(self):
        ...
        try:
            ...
            for ip in self.get_proxies():
                self.useful_proxies[ip] = 0
            logging.warning("总共：" + str(len(self.useful_proxies)) + 'IP可用')
            return self.useful_proxies
        except :
            logging.warning("获取代理ip时出错！")
            return {}
